...
def test_flush_preserva_a_config_dos_terminais(loja_configurada, monkeypatch):
    ...
    device = DeliveryDevice.objects.create(label="Dispositivo preservado", identification="CONFIG-READER", active=False)
    ...
    assert (device.ref, device.label, device.identification, device.active, device.current_order_id) == (
        device_ref, "Dispositivo preservado", "CONFIG-READER", False, None)
