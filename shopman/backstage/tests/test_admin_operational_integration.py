...
class RecipeAdminSemanticsTests(TestCase):
    def test_recipe_admin_edits_ingredients_as_canonical_tabular_inline(self) -> None:
        Product.objects.create(sku="FARINHA-NOVARA-T55", name="Farinha T65", unit="kg")
        recipe = Recipe.objects.create(
            ref="massa-base",
            name="Massa Base",
        ...
        self.assertIn("FARINHA-NOVARA-T55", sku_choices)
        ...
        item = RecipeItem(recipe=recipe, input_sku="FARINHA-NOVARA-T55", quantity=Decimal("1"), unit="zufts")
        with self.assertRaises(ValidationError):
            item.full_clean()

        mismatched = RecipeItem(recipe=recipe, input_sku="FARINHA-NOVARA-T55", quantity=Decimal("100"), unit="g")
        with self.assertRaises(ValidationError):
