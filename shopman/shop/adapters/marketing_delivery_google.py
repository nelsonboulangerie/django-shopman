...
def send(*, artifact, target_key: str, idempotency_token: str) -> ProviderOutcome:
    ...
    try:
        ...
        if exc.status == 401 and _oauth_configured(config):
            _invalidate_cached_token(config, rejected_token=access_token)
            raise ProviderCallFailure(
                ProviderOutcomeKind.FAILED_RETRYABLE,
                "google_publish_oauth_token_rejected",
            ) from None
    ...
    *, target_key: str, idempotency_token: str, provider_receipt_ref: str
    ...
    try:
        ...
        if exc.status == 401 and _oauth_configured(config):
            ...
            raise ProviderCallFailure(
                ProviderOutcomeKind.FAILED_RETRYABLE,
                "google_lookup_oauth_token_rejected",
            ) from None
...
def _config() -> dict:
    ...
    return {
        ...
        "token_url": str(
            raw.get("token_url") or "https://oauth2.googleapis.com/token"
        ).strip(),
...
def _has_credentials(config: dict) -> bool:
    if _oauth_fields_present(config):
        return _oauth_configured(config)
    return bool(config["access_token"])


def _oauth_fields_present(config: dict) -> bool:
    return any(
        (config["client_id"], config["client_secret"], config["refresh_token"])
    )


def _oauth_configured(config: dict) -> bool:
    return bool(
        config["client_id"]
        and config["client_secret"]
        and config["refresh_token"]
        and _secure_token_url(config["token_url"])
    )


def _secure_token_url(value: str) -> bool:
    try:
...
def _access_token(config: dict) -> str:
    """Return a renewable bearer token, retaining static-token canary support."""

    if not _oauth_fields_present(config):
        return config["access_token"]
    if not _oauth_configured(config):
        raise ProviderCallFailure(
            ProviderOutcomeKind.NOT_ATTEMPTED,
            "google_oauth_configuration_invalid",
        )
    ...
    # The provider refresh is kept inside the lock deliberately: concurrent
    # delivery workers in this process must not stampede the token endpoint.
    with _TOKEN_LOCK:
        now = time.monotonic()
        cached = _TOKEN_CACHE.get(cache_key)
        if cached and cached[1] > now:
...
def _invalidate_cached_token(config: dict, *, rejected_token: str) -> None:
    """Drop only the token that was actually rejected by the provider."""
