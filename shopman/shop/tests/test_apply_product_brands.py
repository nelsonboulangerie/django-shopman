...
def test_casa_leva_a_marca_da_loja_e_revenda_a_do_fabricante(shop):
    ...
    assert get_social_attributes(Product.objects.get(sku="CT")).brand == "Nelson Boulangerie"
    camembert = get_social_attributes(Product.objects.get(sku="QC"))
...
def test_sem_apply_nao_grava(shop):
    ...
    assert "CT" in out.getvalue()
    assert get_social_attributes(Product.objects.get(sku="CT")).brand == ""


def test_curadoria_do_gestor_vence_e_sai_no_relatorio(shop):
    _product("QC", social={"brand": "Président", "gtin": "3228020355741"})

    report = apply_brands(apply=True)

    attrs = get_social_attributes(Product.objects.get(sku="QC"))
    assert (attrs.brand, attrs.gtin) == ("Président", "3228020355741")
    assert {(sku, field) for sku, field, *_ in report["conflicts"]} == {("QC", "brand"), ("QC", "gtin")}


def test_preserva_o_resto_do_metadata_e_e_idempotente(shop):
    _product("CT", fiscal={"ncm": "19059090"}, social={"hashtags": ["croissant"]})

    apply_brands(apply=True)
    segunda = apply_brands(apply=True)

    product = Product.objects.get(sku="CT")
...
def test_a_tabela_so_nomeia_skus_que_o_seed_cria(monkeypatch):
    ...
    croissant = get_social_attributes(Product.objects.get(sku="CT"))
