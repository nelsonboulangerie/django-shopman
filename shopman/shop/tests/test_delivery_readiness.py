...
· empurrar para a TV da loja não depende de terceiro nenhum.
...
def test_each_platform_is_classified_by_how_it_delivers(no_transport):
    states = _by_platform(dr.readiness_for(["instagram", "whatsapp", "tv"]))

    assert states["instagram"].kind == dr.PUBLICATION
    assert states["whatsapp"].kind == dr.DIRECT_MESSAGE
    assert states["tv"].kind == dr.LOCAL_DISPLAY


def test_the_store_tv_is_always_ready(no_transport):
    """Tela nossa, push nosso: não há terceiro para bloquear."""
...
def test_the_order_asked_is_the_order_returned(no_transport):
    states = dr.readiness_for(["tv", "whatsapp", "instagram"])
    assert [s.platform for s in states] == ["tv", "whatsapp", "instagram"]
...
def test_a_fully_ready_platform_produces_no_noise(with_transport):
    ...
    template = AnnouncementTemplate.objects.create(name="T", body="oi")
    Campaign.objects.create(
        name="TV", trigger=Trigger.MANUAL, template=template,
        platforms=["tv"], is_active=True,
    )
