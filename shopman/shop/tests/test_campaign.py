...
class TestEvaluate:
    ...
    def test_other_trigger_is_ignored(self, product, rule):
        assert campaign.evaluate("low_stock", _context()) == []
...
class TestTriggerFilter:
    ...
    def test_missing_quality_is_treated_as_bom(self, product, rule):
        ...
        context = _context()
        context.pop("quality")
        assert len(campaign.evaluate("production_finished", context)) == 1

    def test_collection_filter_matches(self, product, rule):
        rule.trigger_filter = {"collections": ["paes"]}
        rule.save()
        assert len(campaign.evaluate("production_finished", _context(collections=["paes"]))) == 1

    def test_collection_filter_excludes(self, product, rule):
        rule.trigger_filter = {"collections": ["paes"]}
        rule.save()
        assert campaign.evaluate("production_finished", _context(collections=["doces"])) == []
    ...
    def test_max_remaining_gate(self, product, template):
        ...
        assert campaign.evaluate("low_stock", _context(available_qty=5)) == []
        assert len(campaign.evaluate("low_stock", _context(available_qty=2))) == 1

    def test_empty_filter_always_matches(self, product, rule):
        assert len(campaign.evaluate("production_finished", _context())) == 1
...
class TestDispatch:
    def test_whatsapp_becomes_a_notify_directive(self, product, template):
        ...
        campaign.evaluate("production_finished", _context())
        assert Directive.objects.filter(topic=ANNOUNCEMENT_NOTIFY).count() == 1
    ...
    def test_vip_first_splits_into_two_waves(self, product, template):
        ...
        campaign.evaluate("production_finished", _context())
        waves = sorted(
            d.payload["wave"] for d in Directive.objects.filter(topic=ANNOUNCEMENT_NOTIFY)
        )
    ...
    def test_the_general_wave_is_delayed(self, product, template):
        ...
        campaign.evaluate("production_finished", _context())
        general = Directive.objects.get(topic=ANNOUNCEMENT_NOTIFY, payload__wave="general")
        vip = Directive.objects.get(topic=ANNOUNCEMENT_NOTIFY, payload__wave="vip")
