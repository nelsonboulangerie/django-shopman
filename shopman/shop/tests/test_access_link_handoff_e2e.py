...
@pytest.mark.django_db
def test_elo_9_o_codigo_e_achado_em_qualquer_campo_do_corpo(joyce):
    """O `access_code` veio errado, mas o código está em OUTRA variável do corpo.

    É o desenho que tira do lojista a adivinhação: manda-se duas ou três variáveis
    candidatas e a que tiver o código vence.
    """
    out = _start(cart_key="cart-scan")
    code = out.get("code") or ""
    IG = "https://lookaside.fbsbx.com/ig_messaging_cdn/?asset_id=17977763199107464"

    r = _post({
        "subscriber": {"id": "mc-joyce", "whatsapp_id": "5543999990009"},
        "access_code": IG,                      # a variável errada (Instagram)
        "custom_fields": {"text": f"#menu {code}"},  # a certa, em outro campo
    })
    body = r.json()
    print("\n[E9] código em campo alternativo →", r.status_code,
          "has_context=", body["has_context"])
    assert body["has_context"] is True, "a sacola tinha de ser achada mesmo assim"
    assert body["handoff_expired"] is False


@pytest.mark.django_db
def test_elo_10_campo_desconhecido_tambem_serve(joyce):
    """Nem precisa ser um nome que a gente conheça: basta o código estar no corpo."""
    out = _start(cart_key="cart-scan-2")
    code = out.get("code") or ""
    r = _post({
        "subscriber": {"id": "mc-joyce", "whatsapp_id": "5543999990009"},
        "shopman_last_message": f"#menu {code}",  # nome inventado pelo lojista
    })
    body = r.json()
    print("[E10] código em campo de nome livre →", r.status_code,
          "has_context=", body["has_context"])
    assert body["has_context"] is True


@pytest.mark.django_db
def test_elo_11_corpo_sem_codigo_nenhum_segue_sendo_login_organico(joyce):
    """A varredura não pode inventar código onde não há."""
    r = _post({
        "subscriber": {"id": "mc-joyce", "whatsapp_id": "5543999990009"},
        "access_code": "#menu",
    })
    body = r.json()
    print("[E11] '#menu' seco →", r.status_code, "handoff_expired=", body["handoff_expired"])
