...
def test_holds_materialized_notifies_customer_with_deadline_and_cart_url():
    ...
    assert kwargs["event"] == "stock_arrived"
...
def test_stock_arrived_renders_a_real_message_in_every_channel():
    ...
    escondia que nenhum canal tinha template de stock_arrived — o WhatsApp
    real mandava o fallback genérico "Notificação: stock_arrived")."""
    ...
    for templates in (MANYCHAT_TEMPLATES, SMS_TEMPLATES, BODY_TEMPLATES):
        ...
        assert f"Confirme até as {local_deadline}" in body
        assert "https://shop.example/sacola" in body
        assert "{" not in body, f"placeholder sem valor no corpo: {body}"
        assert "Notificação:" not in body

    subject = SUBJECT_TEMPLATES["stock_arrived"].format(**context)
