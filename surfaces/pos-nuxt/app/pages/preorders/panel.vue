...
// existe onde estão a bobina e o agente do balcão. O `useCounterAgent` lê a
// sua configuração de `POSProjection.cash_drawer` (`agent_url` + `token`), que
// é identidade de ESTAÇÃO — o Gestor não tem nenhuma. A permissão, porém, é a
// do pedido (`shop.manage_orders`, que o grupo Caixa já tem), porque o
// documento é do pedido e não do caixa.
import {
  PRESET_LABELS,
  activePreset,
  batchNotice,
...
useHead({ title: "Via Pedido do painel" });
...
<template>
  <PosPreordersShell :pos="pos" :pending="posPending" @refresh="refreshPos()">
    <div class="flex flex-wrap items-baseline justify-between gap-2">
      <h1 class="text-lg font-semibold">Via Pedido – painel</h1>
      <span class="text-sm text-muted-foreground">{{ rangeLabel(tickets.range.value, tickets.today) }}</span>
    </div>
    <p class="text-sm text-muted-foreground">
      A Via Pedido de cada encomenda do período, para pendurar no painel.
    ...
    >
      ...
      <p class="text-sm text-muted-foreground">
        Nenhuma encomenda para {{ rangeLabel(tickets.range.value, tickets.today) }}. Escolha outro período acima.
      </p>
    </section>
      ...
      <ul class="grid gap-2">
        ...
        >
          ...
          <UiButton
            variant="ghost"
            size="icon-sm"
            :aria-label="`Imprimir a Via Pedido de ${row.ref}`"
