...
test.describe("Produção — gate de operador", () => {
  ...
  test("sessão autenticada → grade + rail canônico (kit), sem login", async ({ page, context }) => {
    ...
    await expect(page.locator('aside[aria-label="Barra do app Produção"]')).toBeVisible();
    await expect(page.getByRole("button", { name: /barra/i })).toBeVisible();
  });
});

test.describe("Produção — menuboard paralelo aposentado", () => {
  test("/menuboard nega a rota e nunca consulta o storefront", async ({ page }) => {
    const storefrontRequests: string[] = [];
    page.on("request", (request) => {
      if (request.url().includes("/storefront/")) storefrontRequests.push(request.url());
    });
...
test.describe("Produção — borda HTTP", () => {
  test("documento e BFF são privados, não enquadráveis e preservam cookie/Vary", async ({ request }) => {
    const document = await request.get("/", { headers: { "x-forwarded-proto": "https" } });
    expect(document.headers()["content-security-policy"]).toContain("frame-ancestors 'none'");
    expect(document.headers()["content-security-policy"]).toContain("object-src 'none'");
    ...
    expect(document.headers()["strict-transport-security"]).toMatch(/^max-age=31536000/);
