...
test('manifesto, service worker e metadados iOS são servidos pelo build', async ({ page, request }) => {
  const manifestResponse = await request.get('/manifest.webmanifest?v=5')
  ...
  await page.goto('/')
  await expect(page.locator('link[rel="manifest"]')).toHaveAttribute('href', '/manifest.webmanifest?v=5')
  await expect(page.locator('link[rel="apple-touch-icon"]')).toHaveAttribute('href', '/pwa/apple-touch-icon-180x180.png?v=5')
