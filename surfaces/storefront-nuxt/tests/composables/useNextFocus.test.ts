  ...
  const Page = defineComponent({
    setup () {
      ...
      return () => h('main', [
        ...
        h('section', { 'data-focus-target': 'when' }, [h('button', { id: 'today' }, 'Hoje')]),
        h('p', { id: 'error', role: 'alert' }, 'Escolha a data.')
      ])
    }
  })
...
function lastScroll () {
  ...
  return call ? { element: scrolledSpy.mock.contexts.at(-1) as Element, options: call[0] as ScrollIntoViewOptions } : null
}
...
describe('useNextFocus — a página segue o foco', () => {
  it('quando a chave muda, leva o bloco à linha de foco e o foca', async () => {
    ...
    expect(when.getAttribute('tabindex')).toBe('-1')
  ...
  it('bloco com data-focus-control entrega o foco ao controle (a próxima ação é digitar)', async () => {
    ...
    page.key.value = 'contact'
    await settle()

    expect(document.activeElement?.id).toBe('name')
    expect(lastScroll()?.element).toBe(document.querySelector('[data-focus-target="contact"]'))
    page.unmount()
  })
  ...
  it('na montagem não salta se o bloco já está inteiro na área visível', async () => {
    ...
    expect(document.activeElement).not.toBe(document.querySelector('[data-focus-target="when"]'))
    page.unmount()
  })
  ...
  it('chave vazia ou repetida não mexe na página', async () => {
    ...
    page.key.value = 'contact'
    await settle()
    // contact → null → contact: o bloco voltou a ser o foco, então rola uma vez;
    // null em si nunca rola.
  ...
  it('reveal explícito mostra um elemento (center) sem mover o foco quando pedido', async () => {
    ...
    expect(lastScroll()).toEqual({ element: document.getElementById('error'), options: { block: 'center', behavior: 'smooth' } })
