...
describe('surface UX guardrails', () => {
  ...
  it('keeps login copy and recovery actions projection-driven', () => {
    ...
    expect(read('app/composables/useWhatsappVerify.ts')).toContain('settleCart')
  ...
  it('keeps auth routes themed without letting shell home clobber auth/cart state', () => {
    ...
    expect(access).toContain('trySystemBrowserHandoff')
    expect(access).toContain('isInAppBrowser()')
    expect(access).toContain('systemBrowserUrl(handoff.url)')
    expect(access).toContain("apiPath('/api/v1/auth/handoff/')")
  ...
  it('uses scaffolded UI Thing components for OTP, empty states and structured fields', () => {
    ...
    expect(upsellRail).toContain('class="shop-photo-frame shop-photo-frame-sm"')
    expect(upsellRail).toContain('class="block w-full rounded-none"')
    ...
    expect(upsellRail).toContain('class="aspect-[4/3] h-auto w-full rounded-none"')
