...
7. No web, um novo aviso exige entrada pela identidade canônica. A tela preserva a página e o SKU, volta ao mesmo produto e pede confirmação explícita; telefone digitado em POST anônimo é ignorado e não cria assinatura. A migração preserva registros e recibos web antigos sem `customer_ref`, mas os marca `legacy_unverified`: eles deixam de gerar entregas e suas capacidades deixam de autorizar gestão. A pessoa pode criar uma assinatura nova após entrar. O cliente gerencia avisos novos pelo link recebido ou pelas Preferências da conta. O link usa uma capacidade no fragmento, removida da barra ao abrir; não copie a capacidade para logs ou tickets. `GET` consulta; pausa/retomada usam `PATCH`; cancelamento usa `DELETE` e é irreversível.
