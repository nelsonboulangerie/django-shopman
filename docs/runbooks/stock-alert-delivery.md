...
7. O cliente gerencia o aviso pelo link recebido ou pelas Preferências da conta. No aparelho que cadastrou um aviso anônimo, o Storefront recupera o mesmo link da sessão após reload; referência, SKU e contato precisam conferir. O link usa uma capacidade no fragmento, removida da barra ao abrir; não copie a capacidade para logs ou tickets. `GET` consulta; pausa/retomada usam `PATCH`; cancelamento usa `DELETE` e é irreversível.
