...
| `danfe_printed_at` | `string` | `POSDanfeEscposView` (`_stamp_first_print`); `OrderDanfeEscposView` (`order_danfe.claim_print`, sob lock) | `POSDanfeEscposView`/`OrderDanfeEscposView` (decisão de REIMPRESSÃO); `order_danfe.danfe_state` (a DANFE da entrega sai sozinha só enquanto a chave está vazia) | ISO datetime da PRIMEIRA composição da DANFE em bobina (`danfe-escpos`), no PDV ou no Gestor — um carimbo só para o mesmo papel. Mesma semântica de `receipt_printed_at` — o servidor decide a reimpressão, a tela não chuta |
