...
| `kit_packaging` | `bool` | `manage.py apply_grocery_catalog` (`_ensure_packaging`: `<SKU>-EMB`, "Caixa Presente <nome> (embalagem)") | `shop/services/fiscal._kit_lines` (recebe o ágio) · `shop/adapters/catalog.expand_bundle` (fica FORA da expansão de disponibilidade e reserva) | **A caixa física do kit**, componente declarado dele. Não vendável avulsa, sem listagem, `base_price_q=0` (o valor dela na nota é a diferença), `availability_policy=demand_ok` e sem estoque obrigatório. Fiscal: perfil `standard` (sem ST; a embalagem é da casa), sem GTIN, NCM por material, configurável por kit (`GiftBox.packaging_ncm`): **4819.20.00** caixa dobrável de papel/cartão não ondulado (padrão), 4819.10.00 papel/cartão ondulado, 4602.19.00 cesta de vime. |
