...
| `expires_at` | `string` | display | `payment.initiate()` | PaymentStatusView (expiração), PDV (`_pos_payment_response` → "Vale até …"), `payment.timeout` (agendamento) | ISO datetime (tz-aware) de vencimento da cobrança — o QR do **Pix** e o **link de pagamento**. No link é UM relógio escrito nos dois lados: o mesmo instante vai ao Stripe (`Session.create(expires_at=...)`) e ao `PaymentIntent.expires_at`; validade em `SHOPMAN_PAYMENT_LINK_TTL_HOURS` (30 min–24 h). Vencido, a Directive `payment.timeout` pergunta ao gateway e cancela (`payment_expired`); a reconciliação diária acusa o que escapar (`expired_payment_link`) |
