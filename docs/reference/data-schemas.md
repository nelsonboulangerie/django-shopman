...
|-------|------|-------------|----------|-----------|
| `customer` | `dict` | CheckoutView, POS, API (`set_data`), iFood webhook | CommitService, handlers | Dados do cliente: `{name, phone, notes, ref, price_tier, cpf, address}`. **No pedido do iFood** (`shop/services/ifood_orders._map_customer`) vêm mais seis, todos do objeto `customer` deles: `ifood_customer_id` (o `customer.id` — identificador do CLIENTE, e a chave de identidade de `shop/services/customer._handle_ifood`; até 19/09/2026 a estratégia usava o id do PEDIDO e criava um `Customer` por compra), `phone_localizer` e `phone_localizer_expires_at` (o `phone.number` é a central 0800 do iFood; o localizador é o código que se digita lá para chegar ao cliente, e tem prazo — quem lê é `backstage/projections/ifood.contact_relay`), `document`/`document_type`, `orders_count_on_merchant` (pedidos anteriores desta pessoa NESTA loja pela contagem do iFood; `None` = o iFood não informou, que é diferente de `0` = primeira compra) e `segmentation` (a Super-Segmentação do iFood — ⚠️ **dado confidencial pela documentação deles: não vai para o cliente nem para terceiros**; fica no snapshot para leitura interna). ⚠️ `customer.phone` do iFood **não** vira telefone de cadastro: é número de terceiro |
