...
| `gtin_source` | `str` | `manage.py apply_grocery_catalog` (campo `gtin_source` da tabela) | quem cura o catálogo (leitura humana; nenhum código decide por ela) | **De onde veio o GTIN, quando NÃO foi da NF-e nem da embalagem.** Hoje só `"web, a confirmar na embalagem"`: duas fontes públicas ou mais concordando, e ninguém leu o código no produto. ⚠️ Fontes podem errar juntas — quem confere o pote apaga a chave. Ausente = o GTIN veio da nota ou da embalagem. |
...
| `gtin` | `str` | admin (Buyman) | `shop/services/sku_records.start_selling` · `shop/adapters/purchase_invoice_nfe._material_by_catalog_gtin` | Código de barras da embalagem. Vai para `Product.metadata.social.gtin` quando o item passa a ser vendido ("Vender também"), e casa a linha da NF-e sem de-para (sugestão pelo código, junto com o `social.gtin` do produto do mesmo SKU). |
