...
| `delivery_address_structured` | `dict` | CheckoutView (`set_data`) · `shop/services/ifood_ingest.py` | CommitService · `backstage/projections/order_queue._delivery_address` · `backstage/services/receipt_escpos._delivery_lines` | Endereço estruturado do Google Places: `{route, street_number, complement, neighborhood, city, state_code, postal_code, place_id, formatted_address, delivery_instructions, is_verified, latitude, longitude}`. **Do iFood vêm só três**: `complement`, `delivery_instructions` (o `reference` do cliente — é o que as duas telas de despacho imprimem sob "Referência:") e `postal_code`; o endereço formatado NÃO é repetido aqui, porque ele já é `delivery_address` e dois donos do mesmo texto divergem. Ausente no pedido iFood cujo endereço não traz nenhum dos três |
