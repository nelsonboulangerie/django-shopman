...
| `kit` | `bool` | `manage.py apply_grocery_catalog` (caixas presente, tabela `GIFT_BOXES`; o `seed` chama a mesma função) | `apply_grocery_catalog._apply_gift_box` (portão da venda) · leitura humana no catálogo | **O produto é um kit montado na casa** (caixa presente: pães e, às vezes, geleia, antepasto, suco, vinho — decisão do dono, 24/09/2026). A composição mora em `offerman.ProductComponent` (`parent` = o kit), não aqui. Na NFC-e o kit sai **aberto**: `shop/services/fiscal._build_fiscal_items` troca a linha de qualquer produto com componentes por uma linha por componente (qty = qty da linha × qty do componente; valor = total da linha rateado pelo `base_price_q × qty` de cada componente, rateio igual se todos forem zero, resto de centavos na última linha; fiscal e GTIN do componente; `meta.kit = {sku, name, qty}` aponta a linha de origem). A abertura vale para qualquer produto com componentes, inclusive os pacotes de N unidades do mesmo pão (`BRBB2` sai como "2 × Brioche Burger Bun"); `kit` diz outra coisa: **kit sem componentes não vende** — `is_sellable=False` no produto e na listagem até o dono definir a composição (o comando não religa; é passo do gestor). Ausente = produto comum. |
