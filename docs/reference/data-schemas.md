...
| `gallery` | `list[str]` URLs | seed/admin (Offerman) | `storefront/presentation/product_detail._gallery` → `gallery` da PDP (miniaturas clicáveis) | Fotos **adicionais** do produto, URLs absolutas na casa das imagens (`public/img/products/` do storefront). A foto **principal** continua sendo `Product.image_url` — promover uma foto da galeria = copiar a URL dela para `image_url`; não existe flag de principal aqui, de propósito (uma pergunta, um dono). Lista ausente/malformada = PDP de foto única, sem carrossel. |
