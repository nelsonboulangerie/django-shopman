...
| `fired_qty` | `dict[str, int]` | POS `fire_pos_tab` (`session.save`) | `_tab_payload` (`fired_qty` por item) | QUANTAS unidades de cada `line_id` já foram à cozinha. O `fired_lines` acima responde "já foi?" e não bastava: o PDV tem **uma linha por SKU**, então pedir mais um do mesmo item aumenta a QUANTIDADE de uma linha já enviada — e o dedupe do KDS por `line_id` fazia o fire virar no-op, sem a cozinha nunca saber do segundo item. Cada fire manda só a diferença, sob o id de rodada `<line_id>#r<quanto_já_foi>`. Zerado por `line_id` no unfire. **Não propagado ao Order.data** |
