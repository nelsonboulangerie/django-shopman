...
`fiscal_note_ready`: NFC-e autorizada de pedido da LOJA ONLINE (`SHOPMAN_STOREFRONT_CHANNEL_REF`), agendado por `handlers/fiscal.NFCeEmitHandler._after_authorized`; o contexto leva `danfe_url` e `fiscal_test_note`. Balcão e marketplace não recebem este aviso.
