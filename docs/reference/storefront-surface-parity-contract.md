...
| `COPY-WAITLIST-001` | P1 | Pedido com hold planejado ainda nao materializado (`metadata.planned=true`, ativo, sem `expires_at`) e encomenda futura devem ser apresentados como reserva/encomenda aguardando producao, nunca como disponibilidade de prateleira nem preparo imediato. Para hold planejado, a copy canonica usa "fila de espera" em vez de tentar transformar a data da fornada em disponibilidade. | `OrderTrackingProjection.promise.fulfillment_wait_kind` distingue `planned_batch` e `preorder`; testes de tracking bloqueiam "disponibilidade", "começar o preparo" e "fila da fornada" nesse contexto. |
| `COPY-PAYMENT-AUTH-001` | P1 | Cartao `authorized` significa autorizacao aprovada, nao captura confirmada. A copy pode dizer que o cartao foi autorizado e apontar para acompanhamento/status real do pedido, mas nao deve pedir para aguardar confirmacao do estabelecimento como se o aceite financeiro ainda estivesse pendente. Nao deve dizer pagamento confirmado/recebido, nem usar "finalizando" como eufemismo de captura. | Teste de tracking para `payment_authorized`, defaults omotenashi e contrato Payman `authorized -> captured`. |
| `COPY-PIX-001` | P1 | Pix pendente e Pix confirmado sao estados diferentes. Antes da captura, a copy pede pagamento e explica que o acompanhamento muda apos a confirmacao do Pix; nao trata "usar/copiar o codigo" como pagamento nem fala em resposta do estabelecimento. Se disser que a confirmacao e automatica, precisa nomear o Pix, nao o pedido. Em `post_commit`, pedido aceito + Pix pendente nao volta a pedir confirmacao da loja; em `at_commit`, pedido `new` nao inventa aceite operacional. | Testes de tracking para `payment_pix_ready`, `payment_preparing`, `payment_confirmed`; bloco Pix consome notas via `OrderTrackingCopyProjection`, sem texto hardcoded de pagamento no Vue. |
| `COPY-PAYMENT-ACTION-001` | P1 | Estados em que o cliente ainda precisa pagar (`payment_pix_ready`, `payment_card_ready`) descrevem a acao de pagamento e a proxima espera real; nao prometem preparo antes de pagamento capturado e pedido aceito. | Testes de tracking para pedido `new` e `accepted`, defaults omotenashi sem "começamos a preparar" nesses estados. |
| `COPY-FULFILLMENT-CONTEXT-001` | P1 | Texto compartilhado entre retirada e entrega nao pode presumir entrega. Se a funcao/pagina nao recebe fulfillment, usa vocabulario neutro ("receber", "acompanhamento") ou label vindo da projection. | Teste Nuxt para `paymentMethodHint("cash") == "Pague ao receber"` e varredura de copy operacional. |
| `COPY-LIVE-STATUS-001` | P2 | Copy institucional pode prometer acompanhamento do status do pedido, mas nao "preparo em tempo real" fora do estado/projection `preparing`. | Defaults omotenashi usam "status do pedido em tempo real"; teste de varredura bloqueia a frase antiga. |
