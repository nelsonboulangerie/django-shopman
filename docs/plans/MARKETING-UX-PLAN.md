...
**sintoma**, não a doença. A doença é que **o estado dos canais não tem casa**, então ele
...
Os itens 1, 3 e 4 não são "painel". São **conexão de canal**. Estão ali porque não havia
...
pessoas, nesses canais"*. Nós o obrigamos a montar isso em duas telas, com o texto de um
...
## 1. A pergunta que cada tela responde
...
| **Canais** | "por onde eu consigo falar, e o que falta?" | ao ligar algo, ou quando falha |
...
**um resumo do que Canais sabe**, com um caminho para lá.
...
· **Um cinto de avisos** no topo, curto, quando algum canal está bloqueado ou limitado —
  frase + "ver em Canais". Sem botão de configuração aqui.
...
**O que muda de verdade:** o card ganha **prévia por canal**. Ver a bolha do WhatsApp com a
foto, o nome resolvido e o botão é a diferença entre aprovar no escuro e aprovar sabendo.

---

## 3. Campanhas (absorve Regras e Modelos)
...
4. **Onde** — as plataformas, com o estado de cada uma vindo de Canais (marcar Instagram
...
## 4. Canais (a casa que faltava)

A tela que o app não tem, e que explica todos os remendos.

Um cartão por canal, e o conteúdo do cartão é exatamente o que
...
## 6. Prévia: a peça que muda mais

Uma prévia fiel, por canal, alimentada por **dados reais de um produto escolhido**:
...
1. **Canais** — cria a casa e esvazia o Painel dos remendos (o botão "WhatsApp" e o teste
...
- `shopman/shop/services/delivery_readiness.py` — o cálculo que a tela Canais renderiza
