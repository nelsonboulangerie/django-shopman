...
| P3 | **JSONField é para estado contextual, não para evento.** Foi a lição do handoff: `drawer_openings`/`change_requests` no `metadata` era a regra da casa aplicada fora de lugar. | `docs/plans/completed/HANDOFF-POS-EVENT-LOG.md` §"Por que o log existe" |
