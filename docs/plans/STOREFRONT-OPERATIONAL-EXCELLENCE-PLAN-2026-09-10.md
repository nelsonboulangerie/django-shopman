...
| C05 · Consentimento e identidade | Escrever estado desejado; replay não religa canal. Guestman e política D03 são autoridade; inscrição não autoriza comunicação universal. Reconsultar elegibilidade antes de envio/replay; falha de leitura silencia. A assinatura persistente é separada da ocorrência e do recibo; unicidade é `subscription + occurrence + purpose + channel`. Usar a `Directive` canônica como outbox e a mesma chave em retry. `stock_back` exige novo ciclo indisponível→disponível; `production_ready` exige fornada distinta e QC Normal/Ótimo vendável. Pausa/cancelamento são rechecados até o envio. Resultado remoto incerto não é reenviado às cegas. Aviso depende de fato vendável atual; aceite remoto não prova entrega. Preservar máscaras/step-up e não revelar pedido ou PII alheios na recuperação. |
...
| Opt-out | Criar assinatura e ocorrência sintéticas; revogar consentimento WhatsApp antes do claim; processar a `Directive`. O recibo fica `suppressed`, o adapter recebe zero chamadas e a assinatura permanece inativa para envio. Não há entrega externa real. |
