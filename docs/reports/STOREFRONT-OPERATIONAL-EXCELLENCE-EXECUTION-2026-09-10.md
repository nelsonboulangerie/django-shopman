...
**Implementação técnica autorizada de W00–W10 concluída; aceite G2 ainda não satisfeito.** PostgreSQL 16, Redis 7 e Chromium executaram em laboratório local descartável, sem skips no gate de runtime nem no E2E. O piloto sintético e a inspeção manual de navegador/árvore de acessibilidade estão registrados abaixo. D01–D07 foram aprovadas pelo solicitante em 2026-09-11, com responsabilidade assumida. D07 substituiu o consumo one-shot de “Avise-me” por assinatura persistente até pausa/cancelamento, sem expiração automática; a retenção de recibos e dados históricos continua pendente sob a proteção atual. Houve uma exploração humana assistida em iPhone com VoiceOver e um percurso humano assistido de uso geral/aparelho compartilhado na base online; neste último, a sacola permaneceu e nenhum dado pessoal da pessoa anterior reapareceu. TalkBack foi adiado pelo solicitante. Somente o deployment de rename expressamente autorizado foi executado; não houve rollout do código candidato nem promoção de configuração. Um probe diagnóstico posterior acionou indevidamente um único SMS OTP para o contato sintético e foi interrompido sem usar o código. Não houve pagamento ou outra transação externa. Não se atribui ganho humano a testes automatizados ou às observações assistidas.
...
O diff técnico autorizado está concluído. G2 continua fechado apenas pelos itens que exigem observação ou ambiente autorizado: jornadas humanas estruturadas restantes, TalkBack adiado, retenção de recibos/dados históricos, providers/jobs reais e promoção do perfil de pre-go-live. A vigência da assinatura até pausa/cancelamento está decidida e implementada. Esses itens não são tratados como defeitos comprovados nem como autorização de rollout.
