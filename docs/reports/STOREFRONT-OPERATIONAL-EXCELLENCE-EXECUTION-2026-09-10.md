...
**Implementação técnica autorizada de W00–W10 concluída e publicada no perfil vivo de pré-go-live; aceite G2 ainda não satisfeito.** PostgreSQL 16, Redis 7 e Chromium executaram em laboratório local descartável, sem skips no gate de runtime nem no E2E. O piloto sintético e a inspeção manual de navegador/árvore de acessibilidade estão registrados abaixo. D01–D07 foram aprovadas pelo solicitante em 2026-09-11, com responsabilidade assumida. D07 substituiu o consumo one-shot de “Avise-me” por assinatura persistente até pausa/cancelamento, sem expiração automática; a retenção de recibos e dados históricos continua pendente sob a proteção atual. Houve uma exploração humana assistida em iPhone com VoiceOver e um percurso humano assistido de uso geral/aparelho compartilhado na base online; neste último, a sacola permaneceu e nenhum dado pessoal da pessoa anterior reapareceu. TalkBack foi adiado pelo solicitante. O rename e a publicação técnica no pré-go-live foram executados após autorização explícita; nenhuma configuração produtiva foi promovida. Um probe diagnóstico anterior à publicação acionou indevidamente um único SMS OTP para o contato sintético e foi interrompido sem usar o código. Não houve pagamento ou outra transação externa. Não se atribui ganho humano a testes automatizados ou às observações assistidas.
...
- Base congelada inicial: `88314ff0474ba35c6dc69f58bbc2700b545454a4`. A referência local chamada `origin/main` estava recuada e não representava o GitHub. A `main` do GitHub foi buscada diretamente em 2026-09-11 e apontava para `1138c95eee0862630330328cf3bfe2f0b6424796`, com 248 commits ausentes da candidata. Os commits finais da execução foram assentados sobre essa referência; conflitos foram resolvidos preservando os contratos operacionais mais recentes da `main` e incorporando os requisitos ainda necessários desta execução.
...
|---|---|---|
| W00 | Base congelada, consumidores inspecionados, reproduções E, baseline automatizado e responsabilidades assumidas | Baseline humano J01–J16 permanece pendente; J06 foi observado somente na base online |
...
| W06 | Set explícito, revogação antes do envio, assinatura persistente sem expiração automática, ocorrência semântica, recibo por entrega, controle anônimo entre aparelhos, pausa/cancelamento e auditoria; `Directive` canônica é a outbox; D03/D07 aprovadas | Retenção de recibos/dados históricos, auditoria produtiva de legado e eventual contração permanecem pendentes |
...
| W09 | Superfícies existentes preservadas; storage bloqueado, duas abas, teclado, viewport estreito, zoom 200% e inspeção manual da árvore de acessibilidade passaram; exploração assistida em iPhone permitiu alguma navegação com VoiceOver; J06 foi observado na base online em iPhone compartilhado e coberto por regressões na candidata | Demais J01–J16 estruturadas, baseline contrabalançada, TalkBack e Maps/WhatsApp reais permanecem pendentes; as observações não mediram conclusão sem ajuda nem compreensão |
| W10 | Runtime PostgreSQL+Redis, browser real, build, migração mista, recuo de código com tentativa em voo, volume sintético, runbook e decisões D01–D07 concluídos | Jobs e recuo no ambiente real dependem de autorização; medição humana ainda impede G2 |
...
| E11 · pós-decisão D07 | A vigência provisória de 30 dias contrariava a decisão definitiva; anônimo não controlava o aviso em outro aparelho; a primeira rota de gestão respondeu 200 no Django e 404 no BFF; uma forma base64 não canônica do token era aceita | Sem expiração automática; migração seletiva; capacidade opaca no fragmento; `GET` somente leitura e mutações explícitas; allowlist canônica; parser rejeita qualquer representação adulterada; browser local percorreu ativo→pausado→ativo→cancelado |
| E12 · revisão final de segurança | Uma segunda sessão que repetisse SKU + telefone recebia a capacidade da assinatura anônima existente e podia pausá-la ou cancelá-la | POST anônimo genérico; somente criação, identidade autenticada ou marcador exato já pertencente à sessão concede gestão; repetição cruzada não vincula sessão nem retoma aviso pausado; corrida decide um único criador |
...
- **H04:** claim incerto não é aceite nem entrega. Concorrência/dedupe passaram em PostgreSQL; a política D03 foi aprovada. A vigência da assinatura foi decidida até pausa/cancelamento; retenção dos recibos/dados, anonimização/contração e legado produtivo continuam dependentes de auditoria específica. Responsável: marketing/integrações.
...
D07 foi recebida depois dessa implementação inicial e substitui o comportamento one-shot anterior: a autorização persiste; estoque e fornada geram ocorrências distintas; retry do mesmo fato reaproveita a mesma identidade; pausa/cancelamento/opt-out são rechecados até a chamada externa; QC inelegível bloqueia apenas a ocorrência; e contato de `Avise-me` não autoriza marketing genérico. A decisão e seus limites estão no [ADR-029](../decisions/adr-029-persistent-stock-alert-occurrences.md).

A revalidação posterior encontrou uma lacuna comprovada: `production_ready` ainda podia se tornar elegível no fechamento da fornada, antes de uma confirmação gerencial explícita do QC. O fechamento agora grava somente a ocorrência pendente; a ação gerencial na Expedição registra `QUALITY_REVIEWED` no livro imutável e só então aplica a política canônica do canal. Uma correção é também uma revisão. Mudança de qualidade antes do provider suprime a entrega; depois de aceite externo, preserva o recibo e abre alerta de conciliação. [Evidência do gate de QC](storefront-operational-20260910/d07-qc-review-gate-20260911.txt).

### Decisões registradas

As propostas abaixo consolidam D01–D07 sem criar contrato, regra ou fonte de verdade adicional. A aprovação deve ser registrada nesta seção; autorização de decisão não autoriza produção, rollout, mensagens, cobranças ou transações reais.
...
| D07 · Produto/privacidade/operação | “Avise-me” é assinatura persistente por SKU e tipo de ocorrência até pausa/cancelamento, sem expiração automática. `stock_back` representa um ciclo indisponível→disponível; `production_ready`, uma fornada identificada. Entrega usa `Directive`/recibo existentes, rechecagem tardia, controle anônimo por capacidade opaca, pausa/cancelamento e estado indeterminado sem reenvio cego. Contato específico não concede marketing genérico. | Aprovada e implementada tecnicamente. Retenção de recibos/dados históricos permanece pendente sob a proteção atual. |
...
A recuperação de conveniência usa estados/tentativas/erro dos Directive existentes e OperatorAlert `checkout_convenience_pending`. Aviso com aceite desconhecido fica `indeterminate`, abre `stock_alert_dispatch_unknown` e não se repete automaticamente. `check_stock_alert_delivery_sla` sinaliza fila, claim, retry ou incerteza acima de 10 minutos; `audit_storefront_subscriptions` retorna contagens sem PII e sem mutações. Não foi criada instrumentação de sessão/PII ou analytics paralelo. Taxas de recuperação, conflitos, parciais apresentados e consentimento bloqueado ainda precisam integração aos painéis canônicos e dono de operação antes de G2; logs isolados não demonstram essas taxas.

## Migração, compatibilidade e recuo

1. Aplicar expansões `orderman.0005_idempotency_request_fingerprint`, `orderman.0006_idempotency_fingerprint_database_default` e `storefront.0004_stock_alert_dispatch_claim` e `storefront.0005_stockalertdelivery_stockalertoccurrence_and_more` no ambiente isolado antes do frontend. O ensaio parte das migrações imediatamente anteriores, usa MigrationExecutor, dados legados sintéticos e repetição da expansão: fingerprint legado vazio; claim/aceite legados nulos. Não infere entrega histórica.
...
Para fechar G2: executar a baseline contrabalançada e as jornadas humanas estruturadas ainda ausentes; quando houver Android, realizar a passagem TalkBack adiada. O recorte J06 na candidata passou no iPhone compartilhado. O domínio definitivo foi confirmado, o runbook foi corrigido e a promoção técnica ao perfil vivo de pré-go-live foi concluída em janela autorizada; o recuo de código com tentativa sintética em voo passou. Piloto real, rollout comercial, configuração produtiva e qualquer ação externa permanecem sujeitos a autorização explícita.
...
| D07 · assinatura persistente, gate gerencial de QC, concorrência, resposta perdida, falha parcial e migração mista | Storefront amplo 1.523 passed/6 skips PG; PostgreSQL focado 81 passed, incluindo corrida do gate de QC; Backstage 3.718 passed/46 skips/28 subtests; Craftsman 414 passed/3 skips; núcleo final 145 passed; migração PostgreSQL com recuo/avanço passou; Produção frontend 281/281, typecheck, lint e build; Chromium real anterior 28/28 | [registro inicial](storefront-operational-20260910/d07-final-validation-20260911.txt) · [gate de QC](storefront-operational-20260910/d07-qc-review-gate-20260911.txt) |
| D07 · vigência definitiva e gestão anônima | Storefront 1.535 passed/9 skips PG; PostgreSQL concorrente 6/6; migração PostgreSQL 1/1 com 1.000 recibos; frontend final 557/557 em 61 arquivos, typecheck, lint e build; browser local GET/PATCH/PATCH/DELETE 200; 26 checks remotos verdes no head anterior à revisão final | [vigência e gestão](storefront-operational-20260910/d07-persistent-management-20260911.txt), [fechamento pós-revisão](storefront-operational-20260910/d07-session-management-review-20260911.txt) |
| D07 · ownership entre sessões | Reprodução anterior 1 failed; alertas depois 75/75; Storefront 1.539 passed/10 skips PG; Nuxt 558/558 em 61 arquivos, typecheck, lint e build; corrida PostgreSQL exige um único criador | [revisão de ownership](storefront-operational-20260910/d07-cross-session-ownership-review-20260911.txt) |
| Endereço após checkout/logout/relogin | Direcionada 16/16; Storefront completo 1.536 passed/9 skips PG; base online contém o vínculo e registrou GET/PATCH 200 | [revalidação](storefront-operational-20260910/shared-device-address-revalidation-20260911.txt) |

A revisão final encontrou que o link de gestão anônimo existia apenas na memória da resposta de cadastro. A sessão agora recupera a mesma capacidade após reload somente quando referência, SKU e contato conferem; cliente autenticado recebe atalho explícito para **Conta → Preferências → Avisos de produtos**. Uma referência isolada ou marcador de outro contato retorna 404. O flow configurado do ManyChat persiste `management_note`, inclusive a capacidade restrita à assinatura, como campo personalizado; essa limitação, seus controles e a ausência de alternativa transitória segura para o flow aprovado estão registrados no ADR e no runbook. Não se afirma remoção do segredo no provedor.

Uma revisão de segurança posterior comprovou que o próprio POST anônimo ainda transformava telefone + SKU em autorização: ao deduplicar uma assinatura preexistente, gravava seu marcador numa segunda sessão e devolvia a capacidade. O POST agora é genérico; a transação distingue criação de deduplicação e somente criação, sessão já proprietária ou identidade autenticada libera gestão. Uma segunda sessão não recebe marcador, não recupera capacidade, não retoma pausa e não cancela. O retry após resposta perdida preserva a linha e recupera a capacidade apenas na sessão original. A resposta genérica oferece entrar com aquele WhatsApp para conferir ou reativar nas Preferências, sem afirmar que uma assinatura pausada está ativa. A revisão de 2026-09-14 fechou a dívida de posse no web: cadastro novo exige identidade autenticada, usa o telefone canônico da conta e pede confirmação explícita depois do retorno ao produto; texto livre anônimo não cria assinatura nem prova. [Reprodução e correção](storefront-operational-20260910/d07-cross-session-ownership-review-20260911.txt).
...
O diff técnico autorizado está concluído. Permanecem fora dele: rollout comercial/produção; mensagens, cobranças ou transações externas; J01–J16 estruturadas; TalkBack e validação completa por pessoa usuária de leitor de tela; Maps/WhatsApp reais; volume e jobs produtivos em voo; retenção de recibos/dados históricos e validação do ambiente de exposição. D01–D07 e os papéis responsáveis foram aprovados em 2026-09-11. Não se alega ganho humano a partir destes testes.
...
O solicitante percorreu, com orientação, a troca entre duas identidades sintéticas no mesmo iPhone contra a base online. A pessoa A informou nome, endereço e instrução de entrega, confirmou o pedido que persistiu o endereço e saiu. A pessoa B entrou com outra identidade; a sacola continuou disponível, mas o checkout não mostrou nome, endereço nem instrução da pessoa A. Esse recorte observa J06 na base: PII anterior igual a zero, com preservação do contexto não pessoal. A candidata mantém a prova automatizada do isolamento e agora também do percurso checkout → logout → novo login da mesma pessoa → endereço disponível sem redigitação.

Uma conferência posterior de “nenhum endereço” não reproduziu perda: o PostgreSQL gerenciado contém exatamente um endereço ligado à identidade que confirmou o pedido; a outra identidade do ensaio contém zero. Os logs registram GET da lista e PATCH da etiqueta, ambos 200. Resta como hipótese a consulta da outra identidade ou de uma superfície que não carregou a lista canônica. A regressão direcionada de persistência, isolamento, endpoint e novo login passou 16/16.
...
O percurso recebeu ajuda, não foi contrabalançado com a candidata e não mediu tempo ou compreensão financeira. Ele não sustenta alegação de ganho humano nem fecha G2, TalkBack ou as demais J01–J16. Durante a revalidação, um probe de login acionou um SMS OTP pela Comtele porque `DELIVERY_CHAIN` prevalece sobre o `LogSender` da app spec; o probe foi interrompido e o código não foi usado. Essa divergência operacional permanece pendente de decisão de configuração, sem nova mensagem ou alteração no ambiente.
...
A primeira execução do PR revelou duas falhas adicionais. O gate de silêncio exigiu que quatro degradações preexistentes, em arquivos agora tocados, declarassem a razão de permanecerem silenciosas. O job `purchase-nuxt` reproduziu uma corrida já presente na `main`: o helper podia consumir callbacks de frame antes de o timer montar o campo da gaveta. Após ceder um ciclo de tarefas antes do frame, o caso passou 50 vezes consecutivas; os 93 testes, typecheck, lint e build de `purchase-nuxt` também passaram. O lint completo alcançou o script de reprodução congelado no dossiê; foram aplicadas somente formatação automática e remoção de import sem uso, sem alterar seus sete cenários, e `ruff check .` passou.

A execução seguinte revelou outra corrida comprovada no checkout: ao escolher retirada, a tela avançava enquanto persistia o rascunho e uma leitura iniciada antes podia terminar depois, restaurar uma revisão antiga e provocar `revision_changed` no envio. O servidor preservou a integridade; a interface, porém, exigiria nova confirmação. A candidata passou a serializar essas mutações, aguardar a gravação antes do próximo passo e manter monotônica a revisão aceita, sem descartar datas ou slots de uma resposta tardia. O E2E real completo passou 28/28 após a correção. [Prova antes/depois](storefront-operational-20260910/checkout-revision-race-20260911.txt).

## Fechamento técnico de D07 — 2026-09-11

A revisão final separou autorização, ocorrência e entrega sem criar outra fila: `StockAlertSubscription` guarda a escolha específica; `StockAlertOccurrence` identifica o fato; `StockAlertDelivery` registra o resultado; e `Directive` continua sendo a outbox canônica. O merge de audiência já existente continua responsável por deduplicar, dentro da mesma comunicação de Marketing, um telefone vindo de favoritos e alertas enquanto preserva as razões. Aviso direto de disponibilidade e campanha de Marketing são finalidades distintas.

A migração prefere prova verificada ao consolidar duplicatas, mantém inserts de worker antigo por default de banco e não promove legado sem prova. A revisão encontrou e corrigiu a reutilização indevida de uma linha sem prova, sua exposição nas preferências e um ciclo de estoque que podia ficar aberto após o cancelamento da última assinatura. Concorrência, replay do evento, resposta perdida, claim abandonado, opt-out após enfileirar, indisponibilidade na fronteira do provider e falha parcial têm oráculos explícitos. [Evidência final de D07](storefront-operational-20260910/d07-final-validation-20260911.txt).

A decisão definitiva posterior removeu a expiração automática. A expansão mantém `expires_at` apenas para rollout misto e limpa o prazo exclusivamente de assinaturas verificadas e não revogadas. Cada cadastro e cada mensagem carregam o acesso “Gerenciar este aviso”: uma capacidade opaca no fragmento que a página remove da barra antes de consultar o BFF. `GET` é somente leitura; `PATCH` pausa/retoma; `DELETE` cancela irreversivelmente. A retomada não ressuscita entregas suprimidas. Pausa/cancelamento e o worker usam o mesmo lock de canal, com fronteira honesta: antes do adapter suprime; depois do aceite preserva o recibo. A inspeção local encontrou e corrigiu tanto a omissão da rota na allowlist do BFF quanto a aceitação de uma codificação base64 não canônica. [Prova de vigência e gestão](storefront-operational-20260910/d07-persistent-management-20260911.txt).

A revisão cruzada posterior comprovou que o fechamento ainda liberava `production_ready` sem uma decisão gerencial distinta. O gate foi movido para o livro imutável de produção: fechamento cria `pending`; `QUALITY_REVIEWED` ou `QUALITY_CORRECTED` reavalia a ocorrência pela política do canal; uma corrida no PostgreSQL cria uma única entrega; retry após resposta perdida retorna o mesmo commit; e piora antes do provider suprime o recibo pendente. O mesmo gate PostgreSQL revelou e corrigiu uma consulta antiga de Marketing que tentava aplicar `FOR UPDATE` ao lado anulável de um `LEFT JOIN`. [Evidência complementar](storefront-operational-20260910/d07-qc-review-gate-20260911.txt).

O diff técnico autorizado está concluído. G2 continua fechado apenas pelos itens que exigem observação ou ambiente autorizado: jornadas humanas estruturadas restantes, TalkBack adiado, retenção de recibos/dados históricos e providers/jobs reais. A promoção técnica ao perfil de pré-go-live foi concluída; a vigência da assinatura até pausa/cancelamento está decidida e implementada. Os itens ainda abertos não são tratados como defeitos comprovados nem como autorização de rollout comercial.

## Publicação técnica autorizada no pré-go-live — 2026-09-11

Após autorização explícita, o PR #612 passou novamente pela merge queue na árvore exata de integração e entrou em `main` como `cc135f47c5bcabe62ff43eab1e9bb819f191f92c`. O workflow seletivo publicou `web`, `storefront`, `production` e `purchase` com tags imutáveis ligadas ao merge. O deployment DigitalOcean `0f6dda3d-6bef-43c3-a458-945d9fc1d52f` terminou `ACTIVE`, com 47/47 etapas; o job de release aplicou as oito migrations novas com `OK`.

O smoke pós-deploy correlacionou o digest do `web` deste run antes de consultar o ambiente e passou readiness, cardápio, checkout e SSR. A conferência adicional, somente leitura, obteve 200 em health, readiness, home, menu, sacola, checkout, gestão de aviso, termos e privacidade; o readiness reportou banco, cache, migrations e fila em `ok`; o cardápio continha 44 SKUs e 28 vendáveis. Não foram criados pedido, pagamento, OTP, alerta ou mensagem externa. [Evidência da publicação](storefront-operational-20260910/pre-go-live-deployment-20260911.txt).

O preflight encontrou sete variáveis de Marketing presentes no app vivo e ausentes do espelho versionado. Como o deploy foi exclusivamente por imagens, nenhum `apps update` ocorreu e as variáveis foram preservadas. O follow-up declarou no spec alpha as mesmas chaves, valores públicos e placeholder secreto, sem mudar o ambiente vivo. Produção comercial continua bloqueada pelo perfil `staging`, Pix mock, OTP/captura de teste, providers e gates humanos; esta publicação não mede ganho humano nem fecha TalkBack, piloto estruturado ou retenção definitiva.

Enquanto o registro aguardava a fila, os PRs de terceiros #611 e #615 foram incorporados sem conflito. O deployment seguinte, `239c5fd5-846f-4983-be70-9fb660654c73`, publicou `web` no descendente `29e0737e99a806975df8810b5b03393af83e9f02`, reutilizou a imagem `storefront` já validada e terminou `ACTIVE` com 47/47 etapas. O smoke correlacionado passou novamente. O merge documental #617, em `7faf0259a855e8e4094010dd6bef3deb2c134f29`, produziu manifesto vazio (`components: []`) e smoke verde, sem novo build ou deployment.

## Fechamento da comprovação do telefone no “Me avise” — 2026-09-14

**Defeito comprovado.** Na árvore anterior, o teste público enviou um telefone arbitrário sem sessão autenticada, recebeu sucesso e encontrou uma assinatura ativa com `proof_status=verified`. O hash existente comprovava o texto apresentado, não a posse do número. Isso permitia que outra pessoa autorizasse mensagens para um telefone alheio.

**Correção.** O endpoint web agora responde `401` com a próxima ação de entrar e não grava telefone, marcador ou assinatura anônima. O Storefront removeu o campo livre: preserva caminho, filtros, âncora e SKU no `next` da entrada canônica; ao voltar autenticado, mantém o produto em contexto e pede a confirmação explícita. O POST usa somente o telefone canônico da conta. Conhecer ou forjar `?aviso=<sku>` não produz efeito, pois a mutação continua dependente do clique autenticado. Outros canais continuam usando o serviço existente quando o próprio evento de entrada traz identidade verificada.

A expansão `storefront.0008` marca como `legacy_unverified` somente assinaturas web não revogadas sem `customer_ref`. Linhas, ocorrências e recibos permanecem no mesmo modelo e não são apagados; deixam de gerar entrega ou autorizar gestão. Assinaturas web ligadas a cliente, canais verificados diferentes do web e histórico revogado não mudam. A origem individual de cada linha web antiga sem cliente não pode ser reconstruída do schema atual; portanto não se afirma que todas resultaram de abuso. No perfil pré-go-live, a opção conservadora é exigir novo opt-in autenticado em vez de presumir posse.

O retry autenticado após resposta perdida converge para a mesma assinatura e capacidade. Duas conexões PostgreSQL concorrentes elegem uma única criação. Falha da mutação mantém `aviso` na URL e oferece tentar novamente; sucesso remove somente esse parâmetro. Os oráculos anteriores de claim incerto, falha parcial, opt-out tardio e fronteira do provider permanecem verdes. Não houve provider, OTP, pagamento, pedido ou mensagem real nesta validação.

O recuo seguro mantém o boundary do backend: o frontend pode voltar sozinho e receberá `401` sem criar opt-in; uma imagem `web` anterior reabriria o defeito e não deve ser restaurada. A reversão de dados é vazia por desenho, pois não existe base para promover automaticamente `legacy_unverified`. Em falha de release, manter este patch no `web` e avançar uma imagem corrigida.

[Prova antes/depois, migração, concorrência e gates](storefront-operational-20260910/d07-web-phone-proof-20260914.txt). A retenção definitiva, TalkBack, jornadas humanas estruturadas, providers/jobs reais e rollout comercial continuam pendentes e sujeitos às autorizações já registradas. Nenhum ganho humano foi inferido.
