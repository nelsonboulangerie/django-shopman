...
- Base congelada inicial: `88314ff0474ba35c6dc69f58bbc2700b545454a4`. A referência local chamada `origin/main` estava recuada e não representava o GitHub. A `main` do GitHub foi buscada diretamente em 2026-09-11 e apontava para `1138c95eee0862630330328cf3bfe2f0b6424796`, com 248 commits ausentes da candidata. Os 26 commits finais da execução foram assentados sobre essa referência; conflitos foram resolvidos preservando os contratos operacionais mais recentes da `main` e incorporando os requisitos ainda necessários desta execução.
...
| W06 | Set explícito, revogação antes do envio, assinatura persistente, ocorrência semântica, recibo por entrega, pausa/cancelamento e auditoria; `Directive` canônica é a outbox; D03/D07 aprovadas | Retenção definitiva, auditoria produtiva de legado e eventual contração permanecem pendentes |
...
| D07 · Produto/privacidade/operação | “Avise-me” é assinatura persistente por SKU e tipo de ocorrência. `stock_back` representa um ciclo indisponível→disponível; `production_ready`, uma fornada identificada. Entrega usa `Directive`/recibo existentes, rechecagem tardia de consentimento/disponibilidade, pausa/cancelamento e estado indeterminado sem reenvio cego. Contato específico não concede marketing genérico. | Aprovada e implementada tecnicamente. O prazo atual de 30 dias continua aplicado como proteção provisória; duração final, renovação ou vigência até pausa/cancelamento permanecem decisão pendente e não são requisito fechado. |
...
O diff técnico autorizado está concluído. G2 continua fechado apenas pelos itens que exigem observação ou ambiente autorizado: jornadas humanas estruturadas restantes, TalkBack adiado, retenção definitiva — inclusive a duração da assinatura hoje limitada provisoriamente a 30 dias —, providers/jobs reais e promoção do perfil de pre-go-live. Esses itens não são tratados como defeitos comprovados nem como autorização de rollout.
