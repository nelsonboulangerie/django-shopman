...
def test_read_baseline():
    ...
    for n in (1,10,100,500):
        ...
        for build in (build_order_queue,build_two_zone_queue):
            ...
            observations.append({"n": n,"projection": build.__name__,"queries": len(queries),"holds": sum("stockman_hold" in q["sql"].lower() for q in queries),"bytes": len(json.dumps(result).encode()),"cold_ms": elapsed[0],"p50_ms": statistics.median(elapsed),"p95_ms": sorted(elapsed)[18]})
    print(json.dumps({"platform": platform.platform(),"database": connection.vendor,"samples": 20,"observations": observations}))
