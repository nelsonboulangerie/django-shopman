...
env = os.environ.copy()
env["DATABASE_URL"] = f"postgresql://orders_lab@127.0.0.1:55439/{name}"
env["REDIS_URL"] = ""
env["SENTRY_DSN"] = ""
...
def run(args, expected=0, input=None):
    result = subprocess.run(
        [python, ".orders-lab/run_script.py", "manage.py", *args],
        env=env,
        text=True,
        input=input,
...
with psycopg.connect(f"host=127.0.0.1 port=55439 user=orders_lab dbname={name}") as conn:
    ...
    assert (
        conn.execute(
            "SELECT count(*) FROM django_migrations WHERE app='backstage' AND name='0061_delivery_device'"
        ).fetchone()[0]
        == 1
    )
print(
    "PASS: empty schema round trip; populated inventory rollback refused; row and migration retained. Isolated database preserved.",
    flush=True,
)
