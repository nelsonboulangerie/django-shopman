...
with transaction.atomic():
    ...
    for index in range(10):
        ...
        device = DeliveryDevice.objects.create(
            label=f"Maquininha sintética {index}",
            identification=f"HTTP-LAB-DEVICE-{index}",
            active=index != 9,
            current_order=order,
        )
        if order:
            order.data["dispatch"] = {
                ...
                "device_ref": str(device.ref),
                "device_label": device.label,
                "equipment_out_at": "2026-09-11T12:00:00Z",
                "equipment_out_by": "lab",
            }
