...
def spawn(args, logname, extra=None, cwd=None):
    log = open(root / ".orders-lab" / logname, "w")
    logs.append(log)
...
try:
    spawn(
        [python, "docs/reports/execution/orders-20260910/http_read_lab/run_http_lab.py", "serve"],
        "device-perf-8015.txt",
    )
    for port in ports[1:6]:
        spawn(
            [python, "docs/reports/execution/orders-20260910/process_read_lab/serve.py", str(port)],
            f"device-perf-{port}.txt",
        )
    spawn(
    ...
    (root / ".orders-lab/device-process-result.json").write_bytes(
