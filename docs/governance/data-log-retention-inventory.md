...
|---|---|---|---|---|
| `stdout/stderr` da aplicação e dos workers | mensagens e contexto adicional; chamadas existentes incluem referências a cliente, assinante, sessão, endereço, documento e coordenadas | handlers de logging próprios usam `JsonLogFormatter` ou `PrivacySafeFormatter` — a MESMA barreira nos dois, JSON de produção e legível de desenvolvimento | a aplicação não controla o prazo do coletor da infraestrutura; saída direta de comandos não recebe formatter automaticamente | **Parcial:** confirmar no provedor o destino, acesso e TTL máximo de 180 dias; auditar comandos e não registrar payload bruto na origem |
| Sentry, somente quando `SENTRY_DSN` existe | exceção, rota e contexto de integrações | `send_default_pii=False`, corpo de requisição desativado e `before_send` remove usuário, userinfo da URL, query/fragmento, cabeçalhos não autorizados, PII e segredos — o CAMINHO da URL fica, porque é ele que diz qual chamada falhou | configuração de retenção da organização não está versionada neste repositório | **Parcial:** registrar owner, região/subprocessador e TTL de até 180 dias; provar a configuração fora do código |
...
`ConsoleSender` e `LogSender` **imprimem o código em claro, de propósito** — é o
que torna possível entrar em desenvolvimento sem um SMS real. Eles se declaram
com `reveals_code = True`, e é esse marcador, não o nome da classe, que a trava
de produção consulta: qualquer emissor assim é recusado no startup quando
aparece no caminho efetivo de entrega, esteja ele em `MESSAGE_SENDER_CLASS` ou
em qualquer posição de `DELIVERY_CHAIN`.

Essa trava é do `main` desde o #811 (17/09) — antes ela comparava o nome exato e
conhecia só um dos dois irmãos. Este documento **não** propõe calar os dois
emissores: o risco deles é de ambiente, não de retenção, e quem o contém é a
recusa no boot de produção.
