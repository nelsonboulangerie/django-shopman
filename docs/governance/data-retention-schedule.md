...
| R10 | Código, link de acesso e dispositivo confiável | expiração + 7 dias | eliminar | `auth_cleanup` FIXA os 7 dias: `--days` com qualquer outro valor recusa antes de tocar em dado. Continua sem agendamento — quem o chama é operação, não worker |
| R11 | IP bruto auxiliar de consentimento | coleta; máximo 90 dias | redigir IP, preservando prova sem IP | `purge_consent_ip` existe como comando MANUAL: sem flag só conta, `--apply` apaga só o IP e o prazo conta da coleta (`occurred_at` do evento; a situação atual segue o evento que aponta). O `data_retention` conta pelo mesmo serviço. Não entra no worker: o primeiro `--apply` em produção e qualquer agendamento pedem gate humano separado, e agendar exige antes mudar a página de privacidade, que hoje diz que nada é apagado sozinho |
