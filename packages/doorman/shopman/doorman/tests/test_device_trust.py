...
class TestTrustedDeviceModel:
    ...
    def test_create_for(self):
        ...
        cid = uuid.uuid4()
        device, raw_token = TrustedDevice.create_for("customer", cid,
            user_agent="Mozilla/5.0 (Macintosh; Intel Mac OS X) Chrome/120",
        )
        assert device.subject_id == str(cid)
    ...
    def test_revoke_all_for(self):
        ...
        cid = uuid.uuid4()
        TrustedDevice.create_for("customer", cid)
        TrustedDevice.create_for("customer", cid)

        count = TrustedDevice.revoke_all_for("customer", cid)
        assert count == 2

        active = TrustedDevice.objects.filter(subject_type="customer", subject_id=str(cid), is_active=True
        ).count()
        assert active == 0
    ...
    def test_cleanup_expired(self):
        ...
        cid = uuid.uuid4()
        device, _ = TrustedDevice.create_for("customer", cid)
...
class TestDeviceTrustService:
    ...
    def test_check_no_cookie(self):
        """No cookie → not trusted."""
        request = self._make_request()
        assert not DeviceTrustService.check(request, "customer", uuid.uuid4())
    ...
    def test_check_valid(self):
        ...
        assert DeviceTrustService.check(request, "customer", cid)

    def test_check_wrong_customer(self):
        ...
        assert not DeviceTrustService.check(request, "customer", other_cid)
    ...
    def test_check_disabled(self):
        ...
        assert not DeviceTrustService.check(request, "customer", cid)

    def test_trust_sets_cookie(self):
        ...
        device = DeviceTrustService.trust(response, "customer", cid, request)
    ...
    def test_trust_disabled(self):
        ...
        response = HttpResponse()
        request = self._make_request()
        result = DeviceTrustService.trust(response, "customer", uuid.uuid4(), request)
        assert result is None

    def test_trust_emits_signal(self):
        ...
        try:
            ...
            response = HttpResponse()
            request = self._make_request()
            DeviceTrustService.trust(response, "customer", cid, request)
            assert len(received) == 1
            assert received[0]["subject_type"] == "customer"
            assert received[0]["subject_id"] == str(cid)
